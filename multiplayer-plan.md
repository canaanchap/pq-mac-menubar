...
## 16. Next Vertical Slice Design (Execution Plan)

This section defines the next concrete implementation slice after current auth wiring.

### 16.1 Slice Goal
Ship a usable multiplayer loop with:
- admin can verify users and bootstrap realm data,
- client can create/join guilds with online characters,
- client shows guild activity feed generated by governance/idle loop.

### 16.2 Realm Bootstrap (locked)
- First realm name: **Goobland**
- Initial realm record:
  - `realmId`: `realm_goobland_1`
  - `name`: `Goobland`
  - `status`: `active`
  - `supportsGuilds`: `true`
...
In scope:
- Admin minimal tools:
  - force verify account by email,
  - view/search accounts,
  - create/list/update realms.
- Guild basics:
  - create guild,
  - join guild,
  - leave guild,
  - view guild profile.
- App Multiplayer UI shell:
  - account/login state presentation,
  - realm card for `Goobland`,
  - guild create/join cards,
  - guild summary + member count.
- Procedural guild activity feed:
  - simple server-side generated events,
  - surfaced in app Multiplayer tab.
- Governance loop v0:
  - motion lifecycle scaffold,
  - scheduled “meeting tick” generation.

Out of scope for this slice:
- check-in anti-cheat envelope evaluation,
- password reset/email delivery,
- advanced moderation workflows.

### 16.4 Backend Deliverables

#### API endpoints to implement now
- `POST /api/v1/admin/force-verify` (admin authenticated)
- `GET /api/v1/admin/accounts` (admin authenticated)
- `POST /api/v1/admin/realms/create` (admin authenticated)
...
#### DB additions
- Ensure `realms` contains `realm_goobland_1` seed row.
- Add index coverage for:
  - `guild_members(guild_id, status)`
  - `guild_logs(guild_id, created_at)`
  - `accounts(email, verified)`

#### Cron/worker tasks
- `guild_tick.php` every 15 min:
  - emits 1 procedural log entry per active guild (rate-limited).
- `presence_expiry.php` hourly:
  - expires stale presence rows.

### 16.5 Admin Dashboard Deliverables (`admin.progressquest.me`)

#### Page: Accounts
- table columns: email, public_name, verified, created_at.
- search by email/public name.
- action: “Force Verify” button.

#### Page: Realms
- list existing realms.
- create realm form.
- edit status/supportsGuilds.
- pre-seed `Goobland` visibility.

#### Security baseline
- admin session gate (single admin credential in env for v1).
- all admin actions server-logged.

### 16.6 App Deliverables (Menubar)

#### Multiplayer tab sections (revised)
- Account (login status + verified state).
- Realm:
  - selected realm (default `Goobland` for online character).
- Guild:
  - if none: create/join controls.
  - if member: guild summary, leave action.
- Activity:
  - last N procedural guild logs.

#### Character creation + online constraint
- online toggle allowed only when account is verified and signed in.
- new online characters auto-bind to `realm_goobland_1` unless realm selector is later exposed.

### 16.7 Procedural/Governance Content v0

Data-driven dictionaries (new JSON files in data/mod system):
- `guildProcedureVerbs`
- `guildProcedureNouns`
- `guildProcedureTemplates`

Template examples:
- `{{actor}} raises a point of procedural {{noun}}.`
- `{{actor}} calls for a vote on {{topic}}.`
- `The {{guildType}} debates {{topic}} until tea runs out.`

Generation rules:
- one event per guild tick,
- pick template + substitutions with deterministic RNG seed `(guild_id + day + tick)`,
- write to `guild_logs`.

### 16.8 Acceptance Criteria

Backend/API:
- account appears in admin accounts list,
- force verify flips `verified=1`,
- realm `Goobland` exists and returns from `/realms`,
- guild create/join/leave works with authenticated session.

App:
- verified login can create online character and open multiplayer tab,
- player can create or join guild,
- player sees guild summary + procedural activity feed.

Ops:
- cron scripts run without fatal errors for 24h dry run.

### 16.9 Implementation Order (strict)
1. Admin auth shell + accounts list + force verify action.
2. Realm seed/create/list + lock `Goobland` default in app.
3. Guild create/join/leave/profile endpoints.
4. App Multiplayer tab wiring to those endpoints.
5. Procedural guild log generator + feed display.
6. QA pass + changelog + deployment checklist.
